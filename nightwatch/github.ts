...
const home: NightwatchTests = {
    ...
    "Github login with fake credentials": () => {
        browser
            ...
            .setValue("#login_field", process.env.LOGIN_GITHUB_USERNAME)
            .clearValue("#password")
            .setValue("#password", process.env.LOGIN_GITHUB_PASSWORD)
            .waitForElementVisible("[value='Sign in']")
            .click("[value='Sign in']")
            .assert.textContains("#js-flash-container .flash.flash-error", "Incorrect username or password.")
            .end()
    }
}
